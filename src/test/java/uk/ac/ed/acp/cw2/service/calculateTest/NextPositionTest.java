...
public class NextPositionTest {
    ...
    void testNextPosition() {
        ...
        assertThrows(NullPointerException.class, () -> Calculate.nextPosition(new NextPositionDto(null, 0)));

        // Test 2: move in a full rotation (E, N, W, S), we should end up at start point
        PositionDto p2a = Calculate.nextPosition(new NextPositionDto(start, 0));
        PositionDto p2b = Calculate.nextPosition(new NextPositionDto(p2a, 90));
        PositionDto p2c = Calculate.nextPosition(new NextPositionDto(p2b, 180));
        PositionDto p2d = Calculate.nextPosition(new NextPositionDto(p2c, 270));
        double loopDist = Calculate.calculateDistance(new DistanceDto(start, p2d));
        ...
        for (int angle : new int[]{0, 45, 90, 135, 180, 225, 270, 315, 360}) {
            PositionDto p = Calculate.nextPosition(new NextPositionDto(start, angle));
            double d = Calculate.calculateDistance(new DistanceDto(start, p));
