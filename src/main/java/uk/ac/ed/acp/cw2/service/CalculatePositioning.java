...
public class CalculatePositioning {
    ...
    public static boolean isInRegion(RegionCheckDto regionCheckDto){
        ...
        // Raise 400 bad request if region is not closed. i.e. if the first point does not equal the last.
        if (!areEqual){
    ...
    // Helper function of isInRegion that constructs a polygon with Path2D library method and checks if point is inside the polygon
    private static boolean checkPath2D(PositionDto checkPoint, PositionDto firstPoint, List<PositionDto> vertices){
            ...
            // returns true if the point is inside the shape or if the point lies on the boundary of the shape
            // The boundary check is done by detecting overlap between a very small square around the point and the polygon (intersects(Rectangle2D r) method)
